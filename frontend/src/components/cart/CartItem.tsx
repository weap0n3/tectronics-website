import { Clicker } from '@/components/custom-ui/Clicker'
import { PriceBox } from '@/components/custom-ui/PriceBox'
import { Button } from '@/components/ui/button'
import { useCartStore } from '@/store/cart-store/useCartStore'
import { IProduct } from '@/types/product.interface'
import { X } from 'lucide-react'
import React from 'react'

type TCartItemProps = {
	item: IProduct
}

export const CartItem = React.memo(({ item }: TCartItemProps) => {
	const removeCartItem = useCartStore(state => state.removeCartItem)

	return (
		<div
			key={item.id}
			className='grid lg:grid-cols-[3fr_4fr_2fr_2fr_1fr] grid-cols-[3fr_4fr_1fr] items-center lg:gap-4 lg:p-4 p-2 gap-3'
		>
			<img src={item.photo} alt={item.name} className='object-cover' />
			<div className='space-y-3 flex flex-col col-span-2 lg:col-span-1'>
				<h2 className='text-lg font-bold line-clamp-1'>{item.name}</h2>
				<p className='lg:line-clamp-3 line-clamp-2 text-sm lg:text-base'>
					{item.description}
				</p>
			</div>
			<PriceBox
				item={item}
				classNamePrice='lg:text-2xl font-normal text-[22px]'
				classNameDiscPrice='text-lg'
			/>
			<Clicker item={item} />
			<div className='w-full flex justify-center'>
				<Button
					variant='destructive'
					className='p-3 rounded-full'
					onClick={() => removeCartItem(item)}
				>
					<X />
				</Button>
			</div>
		</div>
	)
})
